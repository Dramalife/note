 ...
 * 	Init;
 * Update : 2020.03.12
 *	Optional facility(Change log location);
 * Update
 *
 */
...
#ifdef	FACILITY_SYSLOG_104
#undef	FACILITY_SYSLOG_104
#endif
#define FACILITY_SYSLOG_104	LOG_LOCAL6
//#define FACILITY_SYSLOG_104	LOG_USER

/* general print function - syslog 
 * (这是一个宏，使用方法和printf一样，为了提供一种方便的记录日志的方式)
 */
#define iec104_debug_out(f,...)	do{\
	syslog(FACILITY_SYSLOG_104 | level, f, ##__VA_ARGS__);\
}while(0)
...
int main(int argc, char **argv)
{
	 ...
	 * level = LOG_DEBUG;  
	 */
	level = LOG_DEBUG;  //config_sample : local6.debug /tmp/local6_debug.log
	 ...
	 */
	openlog(argv[0], options, FACILITY_SYSLOG_104);
	 ...
	 *
	 * Log location
	 * Code : syslog(LOG_USER | LOG_DEBUG, ...);
	 * Config : user.debug /tmp/user_debug.log;
	 */
	syslog(FACILITY_SYSLOG_104 | level, "%s, level=0x%x,options=0x%x. ", "This is an example message. ",level, options);
